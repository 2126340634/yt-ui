  ...
  <view :class="stepsClass" :style="stepsStyle">
    <view v-for="(step, index) in list" :key="index" :class="stepsItemClass(index)" :style="stepsItemStyle(step)">
      <view class="yt-steps--item-icon">
        ...
        <image v-else-if="step.icon" class="yt-steps--item-icon-image" :src="step.icon" mode="aspectFit" />
        <span v-else class="yt-steps--item-icon-text">
          {{ index + 1 }}
        </span>
      </view>
      <span class="yt-steps--item-text">{{ step.text }}</span>
      <view v-if="index < props.list.length - 1" class="yt-steps--item-line" />
    </view>
  </view>
</template>
