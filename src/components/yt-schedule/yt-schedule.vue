...
const selectedCourse = shallowRef<(CourseData & { weekIndex: number; gridIndex: number } & { agendaName?: string; agendaLocation?: string }) | null>(
  null
)
...
function handleCourseLongPress(weekIndex: number, index: number) {
  ...
  uni.showModal({
    ...
    success: res => {
      ...
      const week = weekIndex + 1
      const targetIndex = localAgendas.value.findIndex(agenda => agenda.x === x && agenda.y === y && agenda.z.includes(week))
      if (targetIndex !== -1) {
...
function handleAddAgenda() {
  editFormRef.value.validate((validated: boolean) => {
    ...
    if (repeatWeeks.length) {
      const repeatStr = repeatWeeks.length > 5 ? repeatWeeks.slice(0, 5).join('、') + '...' : repeatWeeks.join('、')
      uni.showToast({
        title: `第${repeatStr}周已存在日程，无法添加`,
        icon: 'none'
      })
      return
    }
...
<template>
  <view :class="scheduleClass" :style="scheduleStyle">
    <!-- schedule-header -->
    <view class="yt-schedule--header" @click="showWeek = !showWeek">
      <view
    ...
    <view class="yt-schedule--table">
      ...
      <view class="yt-schedule--table-time">
        <template v-for="(i, index) in rows" :key="index">
      ...
      <!-- lesson-grid -->
      <yt-swiper v-model="curWeek" class="yt-schedule--table-swiper" :list="weekList" :duration="duration" @change="handleSwiperChange">
        <template #swiper-item="{ index: weekIndex }">
          <view v-if="shouldRender(weekIndex)" class="yt-schedule--table-grid">
            <view v-for="index in gridIndexes" :key="`${weekIndex}-${index}`" class="yt-schedule--table-grid-item">
              <view
              ...
              >
                ...
                <view v-if="getCachedCourse(weekIndex, index)?.isConflict" class="yt-schedule--table-grid-item-conflict">
                  {{ '🟡' }}
                </view>
              </view>
            </view>
            <!-- empty-tip -->
            <view v-if="!hasCourses(weekIndex)" class="yt-schedule--table-grid-empty"> Empty </view>
          </view>
        </template>
      </yt-swiper>
    </view>
    ...
    <yt-popup
      ...
      :header-type="editMode ? 'close-only' : 'none'"
      :header-props="{ title: '自定义日程' }"
      :close-on-overlay-click="!editMode"
      placement="center"
      height="fit-content"
      max-height="50vh"
    >
      ...
      <view v-if="!editMode" class="yt-schedule--popup-preview">
        ...
        <view v-if="selectedCourse?.type === 'course'" class="yt-schedule--popup-preview-course">
          ...
          <view>
            ...
            <view v-for="className in courseClasses" :key="className">
              {{ className }}
            </view>
          </view>
          <yt-button v-if="!selectedCourse?.isConflict" class="yt-schedule--popup-preview-add-agenda" size="small" @click="addExtraAgenda"
            >添加自定义日程</yt-button
          >
        </view>
        <!-- divider-line -->
      ...
      <!-- edit -->
      <yt-form v-else ref="editFormRef" :rules="editFormRules">
        <view class="yt-schedule--popup-edit">
          <view class="yt-schedule--popup-edit-name">
            <view>日程</view>
            <yt-input name="nameInput" theme="classic" placeholder="请输入日程安排" />
          </view>
          <view class="yt-schedule--popup-edit-cycle">
            <view>周期</view>
            <yt-picker type="multi" :items="editState.cycleRange" @change="handleCyclePicked">
              <yt-input name="cycleInput" theme="classic" disabled :value="editState.cycle" />
            </yt-picker>
          </view>
          <view class="yt-schedule--popup-edit-time">
            <view>时间</view>
            <yt-input name="timeInput" theme="classic" disabled :value="editState.time" />
          </view>
