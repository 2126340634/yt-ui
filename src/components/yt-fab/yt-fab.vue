...
<template>
  <view :class="fabClass" :style="fabStyle">
    ...
    <!-- pop-menu -->
    <view v-if="popMenu" :class="popMenuClass">
      <view v-for="(item, index) in menuList" :key="index" class="yt-fab--pop-menu-item" @click="handleMenuItemClick($event, index)">
        <yt-icon v-if="iconMap[item.icon]" class="yt-fab--pop-menu-item-icon" :name="item.icon" :size="9" :width="60" :height="60" />
        <image v-else class="yt-fab--pop-menu-item-image" :src="item.icon" mode="aspectFill" />
