...
const popupStyle = computed(() => {
  ...
  return {
    ...
    '--popup-header-close-justify-content': props.headerProps.closeSide === 'left' ? 'flex-start' : 'flex-end'
  } as CSSProperties
})
...
watch(
  ...
  newVal => {
    ...
    if (newVal) {
      emit('open')
      stateTime = setTimeout(() => emit('opened'), props.duration) as any
    } else {
      emit('close')
      stateTime = setTimeout(() => emit('closed'), props.duration) as any
    }
  },
  { immediate: true }
)
...
<template>
  <view v-if="shouldRender" :class="popupClass" :style="popupStyle">
    <view v-if="headerType !== 'none'" class="yt-popup--header">
      <view v-if="headerType === 'cancel-confirm'" class="yt-popup--header-cancel-confirm">
        <span class="yt-popup--text yt-popup--text-cancel" @click="props.headerProps.cancelFn">
          {{ props.headerProps.cancelName }}
        </span>
        <span class="yt-popup--text yt-popup--text-title">{{ props.headerProps.title }}</span>
        <span class="yt-popup--text yt-popup--text-confirm" @click="props.headerProps.confirmFn">
          {{ props.headerProps.confirmName }}
        </span>
      </view>

      <view v-if="headerType === 'close-only'" class="yt-popup--header-close-only">
        <span class="yt-popup--text yt-popup--text-title">{{ props.headerProps.title }}</span>
        <yt-icon name="Close" :width="12" :height="12" :size="38" @click="() => emit('update:visible', false)"></yt-icon>
      </view>
    </view>
  ...
  <yt-overlay :visible="visible" :z-index="zIndex - 1" :bg-color="overlayColor" @click="handleOverlayClick" />
</template>
