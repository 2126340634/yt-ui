...
const centerInnerStyle = computed(() => {
  return {
    ...
    '--tabbar-center-item-bgColor': activeIndex.value === 4 ? props.centerBgColorLast : props.centerBgColor
  }
})
...
<template>
  <!-- tabbar -->
  <view :class="tabbarContainerClass" :style="tabbarContainerStyle">
    <view
    ...
    >
      <yt-icon
        v-if="activeIndex === index ? item.activeIcon && iconMap[item.activeIcon] : iconMap[item.icon]"
        :name="activeIndex === index ? item.activeIcon : item.icon"
        class="yt-tabbar--icon"
        :size="9"
      />
      <image v-else :src="activeIndex === index ? item.activeIcon : item.icon" class="yt-tabbar--image" mode="aspectFit" />
      <span v-if="props.showText" class="yt-tabbar--text">
        {{ item.text }}
      </span>
    </view>
  </view>
  ...
  <!-- centerIcon -->
  <view v-if="centerIcon" @click="handleCenterClick" :class="centerIconClass" :style="centerIconStyle">
    <view class="yt-tabbar--item-center-inner">
      <view class="yt-tabbar--item-center-inner-items" :style="centerInnerStyle">
        <template v-for="(item, index) in validatedList" :key="index">
          <yt-icon v-if="iconMap[item.icon]" :name="item.icon" class="yt-tabbar--item-center-inner-icon" :size="10" width="100%" height="100%" />
          <image v-else :src="item.icon" mode="aspectFill" class="yt-tabbar--item-center-inner-image" />
        </template>
        <yt-icon v-if="iconMap[centerIcon]" :name="centerIcon" class="yt-tabbar--item-center-inner-icon" :size="10" width="100%" height="100%" />
        <image v-else :src="centerIcon" mode="aspectFill" class="yt-tabbar--item-center-inner-image" />
      </view>
    </view>
  </view>
</template>
