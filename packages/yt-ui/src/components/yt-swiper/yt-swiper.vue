...
const swiperClass = computed(() => {
  return ['yt-swiper', `yt-swiper--theme-${props.showBgColor ? props.theme : 'none'}`, { 'yt-swiper--card': isCard.value }]
})
const swiperStyle = computed(() => ({
  ...
  '--swiper-dots-offset': typeof props.dotsOffset === 'number' ? `${props.dotsOffset}%` : props.dotsOffset,
  '--swiper-border-radius': typeof props.borderRadius === 'number' ? `${props.borderRadius}px` : props.borderRadius
}))
...
function updateDraggerTransform() {
  ...
  draggerTransform.value = isHorizontal.value ? `translate3d(${deltaX}px, 0, 0)` : `translate3d(0, ${deltaY}px, 0)`
}
...
function getCardStyle(index: number) {
  ...
  if (index === curIndex.value - 1) translateValue = `${props.cardGap}%`
  // transform
  const translate = isCurrentCard ? 'translate(0, 0)' : isHorizontal.value ? `translateX(${translateValue})` : `translateY(${translateValue})`
  const rotateValue = isCurrentCard ? '0deg' : `${diff * (isHorizontal.value ? -8 : 8)}deg`
  const translateZ = isCurrentCard ? '0px' : '-50px'
  const scale = isCurrentCard ? Math.min(props.activeCardScale, 1) : Math.min(props.inactiveCardScale, 1)
  const cardTransform = isCard.value ? `${translate} translateZ(${translateZ}) ${rotateType}(${rotateValue}) scale(${scale})` : 'none'
  // shadow
  const boxShadow = isCard.value && (isCurrentCard || isAdjacent) ? '0 2px 4px rgba(0, 0, 0, 0.2), 0 8px 16px rgba(0, 0, 0, 0.2)' : 'none'
  // border-radius
...
function isCurrentActive(index: number) {
  return index === curIndex.value || (curIndex.value === visibleList.value.length - 1 && index === 1) || (curIndex.value === 0 && index === visibleList.value.length - 2)
}
...
function shouldShow(displayIndex: number) {
  ...
  // 循环边界相邻
  if (props.loop && props.list.length > 1) {
    return (current === 0 && (displayIndex === total - 1 || displayIndex === total - 2)) || (current === total - 1 && (displayIndex === 0 || displayIndex === 1))
  }
  return false
}
/** */
...
<template>
  <view :class="swiperClass" :style="swiperStyle" @touchstart="handleTouchStart" @touchmove="handleTouchMove" @touchend="handleTouchEnd" @touchcancel="handleTouchEnd">
    <view :class="swiperDraggerClass" :style="swiperDraggerStyle">
      <!-- Item Container -->
      <view :class="swiperContainerClass" :style="swiperContainerStyle">
        <view class="yt-swiper--container-item" v-for="(item, index) in visibleList" :key="index" @click="handleClick" :style="getCardStyle(index)">
    ...
    <!-- Arrow -->
    <view v-if="showArrow" class="yt-swiper--arrow" :style="arrowStyle">
      <yt-icon v-if="isHorizontal && arrowVisible.start" class="yt-swiper--arrow-left" :name="isArrowDark ? 'ArrowLeft' : 'ArrowLeftWhite'" :size="arrowSize" @click="handlePrev" />
      <yt-icon v-if="isHorizontal && arrowVisible.end" class="yt-swiper--arrow-right" :name="isArrowDark ? 'ArrowRight' : 'ArrowRightWhite'" :size="arrowSize" @click="handleNext" />
      <yt-icon v-if="!isHorizontal && arrowVisible.start" class="yt-swiper--arrow-up" :name="isArrowDark ? 'ArrowUp' : 'ArrowUpWhite'" :size="arrowSize" @click="handlePrev" />
      <yt-icon v-if="!isHorizontal && arrowVisible.end" class="yt-swiper--arrow-down" :name="isArrowDark ? 'ArrowDown' : 'ArrowDownWhite'" :size="arrowSize" @click="handleNext" />
    </view>

    <!-- Dots -->
    <view v-if="showDots" :class="dotsClass">
      <yt-dots :theme="theme" :total="maxSwiperDots" :size="props.dotsSize" :activeIndex="realCurIndex" :direction="props.direction" />
    </view>
  </view>
</template>
