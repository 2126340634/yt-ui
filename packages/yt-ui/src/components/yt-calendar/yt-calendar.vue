...
const calendarGridClass = computed(() => {
  return (item: Calendar, index: number) => {
    const active = calendar.isToday(item.year, item.month, item.date) || index === selectedDate.value?.index
    const secondary = active && selectedDate.value !== null && item.date !== selectedDate.value.item.date
    return {
...
const arrowScale = computed(() => {
  ...
  const rightScale = deltaX < 0 ? 1 + Math.abs(touchState.value.curX - touchState.value.startX) * 0.012 : 1
  const leftScale = deltaX > 0 ? 1 + Math.abs(touchState.value.curX - touchState.value.startX) * 0.012 : 1
  return {
  ...
  <view :class="calendarClass" :style="calendarStyle">
    <view class="yt-calendar--header">
      ...
      <view class="yt-calendar--header-year-month">
        <span @click="changeMonth('prev')" class="arrow-left">
          {{ '<' }}
        </span>
        <!-- date-picker -->
        <yt-picker type="date" fields="month" @change="handleDatePickerChange" :start="start" :end="end">
          <span>{{ `${state.year}年${state.month}月` }}</span>
        </yt-picker>
        <span @click="changeMonth('next')" class="arrow-right">
          {{ '>' }}
        </span>
      </view>
    </view>
    <view class="yt-calendar--body" @touchstart="handleTouchStart" @touchmove="handleTouchMove" @touchend="handleTouchEnd" @touchcancel="handleTouchEnd">
      <view class="yt-calendar--body-week">
        <span v-for="(day, index) in calendar.weekDays" :key="index">
          {{ day }}
        </span>
      </view>
      <view :class="['yt-calendar--body-grid', { 'yt-calendar--body-grid-loading': props.loading }]">
        <span v-for="(item, index) in calendarData" :key="index" :class="calendarGridClass(item, index)" @click="handleDateClick(item, index)">
          <p class="yt-calendar--body-grid-item-date">{{ item.date }}</p>
          <p class="yt-calendar--body-grid-item-extra">
            {{ getExtra(item) }}
          </p>
          <p class="yt-calendar--body-grid-item-holiday" v-if="isHoliday(item)">休</p>
        </span>
        <!-- background-text -->
        <span class="yt-calendar--body-grid-background-text">{{ backgroundText }}</span>
      </view>
      <!-- loading -->
      <yt-loading class="yt-calendar--body-loading" :theme="theme" :visible="loading" />
    </view>
  </view>
</template>
