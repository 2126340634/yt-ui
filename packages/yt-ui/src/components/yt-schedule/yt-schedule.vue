...
<template>
  <view :class="scheduleClass" :style="scheduleStyle">
    ...
    <!-- schedule-week -->
    <scroll-view scroll-x scroll-with-animation :scroll-into-view="enableAutoScrollWeek ? `${'week-' + curWeek}` : ''" :show-scrollbar="false" class="yt-schedule--week">
      <view v-for="(i, index) in weeks" :id="'week-' + index" :key="index" :class="weekTextClass(index)" @click="handleWeekClick(index)"> 第{{ i }}周 </view>
    </scroll-view>
    <!-- schedule-date -->
    <view class="yt-schedule--date">
      <view class="yt-schedule--date-month">{{ weekMonth }}月</view>
      <view v-for="(day, index) in schedule?.weekDays" :class="dateContainerClass(index as number)" @click="handleDateClick(index as number)">
    ...
    <view class="yt-schedule--table">
      ...
      <!-- lesson-grid -->
      <yt-swiper class="yt-schedule--table-swiper" v-model="curWeek" :list="weekList" :duration="duration" @change="handleSwiperChange">
        <template #swiper-item="{ index: weekIndex }">
          <view class="yt-schedule--table-grid" v-if="shouldRender(weekIndex)">
            <view class="yt-schedule--table-grid-item" v-for="index in gridIndexes" :key="`${weekIndex}-${index}`">
              <view
      ...
      <yt-form v-else ref="editFormRef" :rules="editFormRules">
        <view class="yt-schedule--popup-edit">
          ...
          <view class="yt-schedule--popup-edit-cycle">
            <view>周期</view>
            <yt-picker type="multi" @change="handleCyclePicked" :items="editState.cycleRange">
