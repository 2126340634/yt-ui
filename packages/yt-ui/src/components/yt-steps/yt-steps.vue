...
<template>
  <view :class="stepsClass" :style="stepsStyle">
    <view v-for="(step, index) in list" :key="index" :class="stepsItemClass(index)" :style="stepsItemStyle(step)">
      <view class="yt-steps--item-icon">
        <yt-icon v-if="step.icon && iconMap[step.icon]" width="100%" height="100%" :name="step.icon" />
        <image class="yt-steps--item-icon-image" v-else-if="step.icon" :src="step.icon" mode="aspectFit" />
        <span class="yt-steps--item-icon-text" v-else>
          {{ index + 1 }}
        </span>
      </view>
      <span class="yt-steps--item-text">{{ step.text }}</span>
      <view class="yt-steps--item-line" v-if="index < props.list.length - 1" />
    </view>
  </view>
</template>
