...
const fabClass = computed(() => {
  return ['yt-fab', `yt-fab--theme-${props.theme}`, `yt-fab--${props.horizontal}`, `yt-fab--${props.vertical}`]
})
...
<template>
  <view :class="fabClass" :style="fabStyle">
    <view :class="innerCircleClass" @click="handleFabClick">
      <yt-icon name="Plus" width="100%" height="100%" />
    </view>
    <!-- pop-menu -->
    <view :class="popMenuClass" v-if="popMenu">
      <view v-for="(item, index) in menuList" :key="index" class="yt-fab--pop-menu-item" @click="handleMenuItemClick($event, index)">
        <yt-icon class="yt-fab--pop-menu-item-icon" v-if="iconMap[item.icon]" :name="item.icon" :size="9" :width="60" :height="60" />
        <image class="yt-fab--pop-menu-item-image" v-else :src="item.icon" mode="aspectFill" />
