...
<template>
  <view class="yt-collapse" :style="collapseStyle">
    <template v-for="(title, index) in safeList" :key="index">
      <!-- collapse-item -->
      <view @click="toggleExpand(index)" :class="collapseItemClass(index)" :style="collapseItemStyle(index)">
        <yt-icon class="yt-collapse--item-icon" v-if="isIconLeftSide" name="ArrowRight" :size="14" :width="30" :height="30" />
        <span class="yt-collapse--item-title">{{ title }}</span>
        <yt-icon class="yt-collapse--item-icon" v-if="!isIconLeftSide" name="ArrowRight" :size="14" :width="30" :height="30" />
      </view>
      <!-- collapse-item-content -->
      <view :class="collapseContentClass(index)" v-show="isExpanded(index)">
